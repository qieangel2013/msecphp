...
class CrawlService{
    ...
    public function GetMP3List($request, $isJson)
    {
        ...
        // TODO: 业务逻辑实现
        nglog_info("GetList start...\n");
        attr_report('GetList entry');
        for ($i=0; $i <10 ; $i++) { 
            $oneMP3 = new OneMP3();
            $oneMP3->set_url('192.168.102.164');
            $oneMP3->set_title($i);
            nglog_info(sprintf("get one mp3:%s,%s\n",'192.168.102.164', $i ));
            $rsp->set_mp3s($i,$oneMP3);
        }


        $rsp->set_status(0);
        $rsp->set_msg("success");
        nglog_info("GetList successfully\n");
        attr_report("GetList successfully");
